...
def compute_metric(known, novel):
    ...
    # score = roc_auc_score(np.concatenate([np.zeros_like(known),np.ones_like(novel)]), np.concatenate([known,novel]))
    score = roc_auc_score(np.concatenate([np.ones_like(known),np.zeros_like(novel)]), np.concatenate([known,novel]))
#     score = roc_auc_score(np.concatenate([np.zeros_like(known),np.ones_like(novel)]).transpose()[0], np.concatenate([known,novel]).transpose()[0])
    results[stype][mtype] = score
...
def print_results(results):
    ...
    print('')
    return results['AUROC'], results['TNR'], results['DTACC'], results['AUIN'], results['AUOUT']
