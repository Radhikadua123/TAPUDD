...
# "MSP", "ODIN", "Energy", "Mahalanobis", "KL_Div"
for method in ["KL_Div"]:
    mean_AUROC = []
    mean_AUPR_in = []
    mean_AUPR_out = []
    mean_FPR_95 = []
    for seed in [0, 1, 2, 3, 4, 663, 1458, 1708, 1955, 7130]:
        ...
        for var in adjust_scale:
            ...
            if(method == "MOS"):
                with open(f'results/logs_ood_scores/NAS_DETECTION_{adjust_type}_test_{method}/seed_{seed}/cluster2/{var}/ood_scores.log') as f:
                    lines = f.readlines()
                ...
                with open(f'results/logs_ood_scores/NAS_DETECTION_{adjust_type}_test_{method}/seed_{seed}/{var}/ood_scores.log') as f:
                    lines = f.readlines()
                AUROC.append(float(lines[-4].split("AUROC using sk.auc: ")[-1]))
                AUPR_in.append(float(lines[-3].split("AUPR (In) using sk.auc: ")[-1]))
                AUPR_out.append(float(lines[-2].split("AUPR (Out) using sk.auc: ")[-1]))
                FPR_95.append(float(lines[-5].split("FPR95: ")[-1]))
