...
def MahalaEnsemble(args, logger, clusters, logdir, reverse, best):
    ...
    for cluster in clusters:
        ...
        file_path_id_scores = os.path.join(args.ood_scores_path, f"cluster{cluster}", "id_scores.npy")
...
def EnsembleTop8(logger, args, labels, gmm_Df, length_in_egs, reverse=False, best=False):
    ...
    samples_more_than_std = (std[length_in_egs:] > np.mean(np.array(std[length_in_egs:]))).sum()
    samples_more_than_mean_mb = (mean[length_in_egs:] > np.mean(np.array(mean[length_in_egs:]))).sum()

    hist_mean_ood = np.array(mean[length_in_egs:])
    hist_mean_id = np.array(mean[:length_in_egs])

    hist_std_ood = np.array(std[length_in_egs:]) 
    hist_std_id = np.array(std[:length_in_egs]) 
    ...
    print(hist_scaled_mean_ood[:10], hist_mean_ood[:10], hist_std_ood[:10] )
    print(hist_scaled_mean_id[:10], hist_mean_id[:10], hist_std_id[:10] )
    ...
    logger.info('total ood samples: {}'.format(len(out_examples)))
    logger.info('total id samples: {}'.format(len(in_examples)))
    logger.info('samples more than mean std: {}'.format(samples_more_than_std))
    logger.info('samples more than mean mb: {}'.format(samples_more_than_mean_mb))
    logger.info('mean uncertainity of OOD dataset: {}'.format(np.mean(np.array(std[length_in_egs:]))))
    logger.info('mean uncertainity of ID dataset: {}'.format(np.mean(np.array(std[:length_in_egs]))))
    logger.info('mean mahalnobis of OOD dataset: {}'.format(np.mean(np.array(mean[length_in_egs:]))))
    logger.info('mean mahalanobis of ID dataset: {}'.format(np.mean(np.array(mean[:length_in_egs]))))
